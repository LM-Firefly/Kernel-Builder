...
The workflow has a manual `workflow_dispatch` trigger. Use `release_mode=official` for the maintained `alpha`, `mate`, and `smart` channels. Official runs generate a unique tag from the Actions run id, preserving the download count and history of every build. Use `release_mode=custom` to build exactly one channel: provide the source repository/ref, channel id/name, version, and your own Release tag. The custom Release publishes only `kernel-plugin.zip`; after extraction it contains the same `kernel-index.json` and verified kernel archive format as an official build.
